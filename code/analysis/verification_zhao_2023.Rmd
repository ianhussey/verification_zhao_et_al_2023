...
forest(fit_zhao_all, addpred = TRUE)
...
forest(fit_zhao_excl_atjak, addpred = TRUE)
...
comparisons <- 
  ...
  mutate(comparison = case_when(source == "Zhao et al. (2023)" ~ "",
                                ...
                                TRUE ~ "Consistent")) |>
  mutate(comparison = case_when((study == "Ren Zhihong 2012" & source == "Reextracted") ~ "Duplicate",
                                TRUE ~ comparison))
...
compared_es <- comparisons %>%
  ...
  mutate(Comparison = ifelse(comparison == "", lead(comparison), comparison),
         Comparison = fct_relevel(Comparison, "Consistent", "Inconsistent", "Duplicate", "Indeterminable")) |>
...
## Updated meta-analysis of the 10 studies after excluding the duplicate

Following the same workflow as Zhao et al. (2023), a random effects meta-analysis was then fit to the 10 sets of effect sizes (i.e., including the 3 that were indeterminable, on the liberal assumption that they are correct, but excluding the duplicate on the reasonable assumption that it is not).

```{r}

data_reextracted_excluded <- data_reextracted %>%
  filter(study != "Ren Zhihong 2012") %>%
  escalc(measure = "SMD",
         m1i = mean_exp,
         m2i = mean_control,
         sd1i = sd_exp,
...
fit_reextracted <- 
  rma(data = data_reextracted_excluded,
      yi = yi,
      vi = vi,
      slab = study,
      method = "DL")
...
I therefore fitted a new meta-analysis to the remaining 9 studies after excluding Zemestani et al. (2020).

```{r}

data_reextracted_excluded_zemestani <- data_reextracted_excluded |>
  filter(study != "Mehdi Zemestani 2020")

fit_reextracted_excl_zemestani <- 
  rma(data = data_reextracted_excluded_zemestani,
      yi = yi,
      vi = vi,
      slab = study,
      method = "DL")
...
## Bias correction via Robust Bayesian Meta-Analysis

Zhao et al. (2023) employ a sensitivity analysis to remove outlier studies (but not studies flagged in the risk of bias assessment), but they do not employ any bias correction method to correct for sources such as publication bias. Here I use Robust Bayesian Meta-Analysis (RoBMA), an ensemble method of correcting for potential sources of bias.
...
roBMA_model <- RoBMA(d = data_reextracted_excluded_zemestani$yi, 
                     se = sqrt(data_reextracted_excluded_zemestani$vi),
                     effect_direction = "negative",
                     prior_scale = "cohens_d",
                     study_names = data_reextracted_excluded_zemestani$study,
                     parallel = TRUE)


# summary(roBMA_model)

interpret(roBMA_model)

```

Results from the RoBMA suggest that the true effect size may be a) smaller again and b) not detectably different from zero. 
